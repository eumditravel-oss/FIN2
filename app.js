...
(() => {
  ...
  scrolls.forEach((sc) => {
   ...
   // === [ADD] 산출표 컬럼 드래그 리사이즈 ===
function enableColumnResize(gridWrap, options){
  if (!gridWrap) return;

  const storageKey = `FIN_COLWIDTHS_${options.key}`;
  const head = gridWrap.querySelector(".grid-head");
  if (!head) return;

  // 1) 초기 폭 로드
  let widths = null;
  try { widths = JSON.parse(localStorage.getItem(storageKey) || "null"); } catch {}
  if (!Array.isArray(widths) || widths.length !== options.cols.length) {
    widths = [...options.cols];
  }

  // 2) grid-cols 적용
  function applyWidths() {
    const cols = widths.map(w => `${Math.max(50, Math.floor(w))}px`).join(" ");
    gridWrap.style.setProperty("--grid-cols", cols);
  }
  ...
  // 3) 기존 resizer 제거 후 재생성
  gridWrap.querySelectorAll(".col-resizer").forEach(el => el.remove());

  // 4) 각 "열 경계" 위치에 resizer 생성
  const headCells = Array.from(head.children);
  if (headCells.length < 2) return;

  function layoutResizers() {
    gridWrap.querySelectorAll(".col-resizer").forEach(el => el.remove());
    ...
    for (let i = 0; i < widths.length - 1; i++) {
      ...
      rz.style.left = `${accLeft - gridWrap.scrollLeft}px`;
      gridWrap.appendChild(rz);
    }
  }

  layoutResizers();

  gridWrap.addEventListener("scroll", () => layoutResizers(), { passive: true });
  window.addEventListener("resize", () => layoutResizers());
  ...
  gridWrap.addEventListener("pointermove", (e) => {
    ...
    const minW = 50;
    let newA = dragging.wA + dx;
    let newB = dragging.wB - dx;

    if (newA < minW) { newB -= (minW - newA); newA = minW; }
    if (newB < minW) { newA -= (minW - newB); newB = minW; }

    widths[dragging.idx] = newA;
    widths[dragging.idx + 1] = newB;
  ...
  gridWrap.addEventListener("pointercancel", () => {
    if (!dragging) return;
    dragging = null;
    document.body.classList.remove("col-resizing");
    saveWidths();
